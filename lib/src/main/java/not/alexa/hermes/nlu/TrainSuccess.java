...
public class TrainSuccess implements HermesMessage<TrainSuccess> {
	...
	public String getTopic() {
		return "rhasspy/nlu/"+siteId+"/trainSuccess";
	}
	...
	public TrainSuccess forTopic(String topic) throws IllegalTopicException {
		if(topic.startsWith("rhasspy/nlu/")&&topic.endsWith("/trainSuccess")) {
			String siteId=topic.substring("rhasspy/nlu/".length(),topic.length()-"/trainSuccess".length());
			if(siteId.indexOf('/')<0) {
