...
public class Train implements HermesMessage<Train> {
	...
	public String getTopic() {
		return "rhasspy/asr/"+siteId+"/train";
	}
	...
	public ASRError createError(String msg) {
		return new ASRError(msg,siteId);
	}
	...
	public Train forTopic(String topic) throws IllegalTopicException {
		if(topic.startsWith("rhasspy/asr/")&&topic.endsWith("/train")) {
			String siteId=topic.substring("rhasspy/asr/".length(),topic.length()-"/train".length());
			if(siteId.indexOf('/')<0) {
