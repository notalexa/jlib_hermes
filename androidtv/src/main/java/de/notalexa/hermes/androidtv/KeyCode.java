...
/**
 * Represents a keycode for the TV. Two additional keycodes are defined:
 * 
 *<ul>
 *<li>{@link #KEYCODE_TURN_ON} turns the TV on
 *<li>{@link #KEYCODE_TURN_OFF} turns the TV off
 *</ul>
 *
 *As a {@link Message}, the keycode is sent to the TV.
 *
 */
public enum KeyCode implements Message {
	  KEYCODE_UNKNOWN         ( 0),
	  KEYCODE_SOFT_LEFT       ( 1),
	  ...
	  KEYCODE_TURN_ON (1000),
	  KEYCODE_TURN_OFF (1001);
	...
	public static KeyCode fromString(String code) {
		try {
			return valueOf(code.toUpperCase());
		} catch(Throwable t) {
		}
		try {
			return valueOf("KEYCODE_"+code.toUpperCase());
		} catch(Throwable t) {
		}
	...
	public boolean perform(Connection con) throws Throwable {
		switch(this) {
			case KEYCODE_TURN_ON:
			case KEYCODE_TURN_OFF:
				if(con.running!=(this==KeyCode.KEYCODE_TURN_ON)) {
					con.cmd(KeyCode.KEYCODE_POWER);
				}
				break;
			default:
				Thread.sleep(500);
				Handler.LOGGER.info("Send key event {} to {}",code,con.getIp());
				con.writeCommand(
						proto.message(
								proto.struct(10,
										proto.integer(1,getCode()),
										proto.integer(2,1))));
				Thread.sleep(200);
				con.writeCommand(
						proto.message(
